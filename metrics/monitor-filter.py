...
def main():
    parser = argparse.ArgumentParser(description="Monitor filter is \
                                     used to find out the differences among \
                                     the hardware metrics on which the \
                                     subject folder runs")
    parser.add_argument("first_monitor_text")
    parser.add_argument("second_monitor_text")
    args = parser.parse_args()
    if os.path.exists(args.first_monitor_text) and \
       os.path.exists(args.second_monitor_text):
        ...
        if compare_monitor_files(file1_lines) != \
           compare_monitor_files(file2_lines):
            print 1
        else:
            print 0
...
def compare_monitor_files(file_lines):
    ...
    for line in file_lines:
        ...
        if package_flag:
            if "*" not in line:
                # Check if the last line is only computed the checksum.
        ...
        if "Linux" in line:
            line_data = line.strip().split(" ")
            hasher.update(line_data[0] + line_data[2] +
                          line_data[13] + line_data[14])
