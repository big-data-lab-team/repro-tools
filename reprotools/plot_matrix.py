...
def main():
    ...
    parser.add_argument("original_matrix",
                        help="Matrix file to plot. Each row should be "
                             "in the following format: <ignored> <subject_id> "
                             " <binary_difference> <file_id>. "
                             "File ids should be ordered according to "
                             "their latest modification time.")
    # plotting test
    # (ordered_file_id, subject, binary_difference, predict_value)
    parser.add_argument("--test_matrix", "-t",
                        help="Matrix file to plot. Each row should be in"
                             "the following format:"
                             "<file_id> <subject_id> <ignored> <predic_value>."
                             "File ids should be ordered according to "
                             "their latest modification time.")
    ...
    n = np.zeros(shape=(len(original_matrix),
                        len(original_matrix[0])), dtype=int)
    ...
    if args.test_matrix is not None:
        colors = ['#000000', '#FFFFFF', '#4BFC4B', '#777777',
                  '#FFFF00', '#FF0000', '#275EDD']
        d = np.unique(n)
    ...
    if args.test_matrix is not None:
        plt.legend([black, white, green, gray, yellow, red],
                   ["Negative", "Positive", "True Positive",
                    "True Negative", "False Negative", "False Positive"],
                   bbox_to_anchor=(0., 1.06, 1., .102), loc=1, ncol=3,
                   mode="expand", borderaxespad=0., fontsize='x-small')
    # else:
        # plt.legend([black,white],["Negative","Positive"],
        #             bbox_to_anchor=(0.,1.06 , 1., .102), loc=1, ncol=2,
        #             mode="expand", borderaxespad=0., fontsize='x-small')
    plt.xlabel('Subject')
    plt.ylabel('File-id')
